...
public final class RecursiveDescentParser {
    ...
    private MJStatementNode Block() {
        MJBlock currentBlockOfStatements = null;
        check(lbrace);
        List<MJStatementNode> statements = Statements();
        currentBlockOfStatements = new MJBlock(statements.toArray(new MJStatementNode[statements.size()]));
        check(rbrace);
        ...
        switch (sym) {
            ...
            case ident:
                ...
                switch (sym) {
                    ...
                    // break;
                    case lpar:
                        List<MJExpressionNode> parameters = ActPars();
                        ...
                        MJExpressionNode invoke = new MJInvokeNode(callTarget, parameters.toArray(new MJExpressionNode[parameters.size()]));

                        currentlyParsedStatement = new MJStatementExpression(invoke);
                        break;
                    case pplus:
                        scan();
                        break;
            ...
            case if_:
                ...
                currentlyParsedStatement = new MJIfNode(condition, trueBranch, falseBranch);
                break;
            // ----- "while" "(" Condition ")" Statement
            case while_:
                scan();
                check(lpar);
                MJExpressionNode condition_ = Condition();
                check(rpar);
                MJStatementNode statement_ = Statement();
                currentlyParsedStatement = new MJWhileLoop(condition_, statement_);
                break;
            // ----- "break" ";"
            case break_:
                scan();
            ...
            case return_:
                scan();
                MJExpressionNode returnValue = null;
                if (sym != semicolon) {
                    returnValue = Expr();
                } else {
                    break;
                }
                currentlyParsedStatement = new MJReturnNode(returnValue);
                check(semicolon);
                break;
            // ----- "read" "(" Designator ")" ";"
            case read:
            ...
            case print:
                ...
                break;
            case lbrace:
                currentlyParsedStatement = Block();
                break;
            case semicolon:
                scan();
                break;
    ...
    private List<MJExpressionNode> ActPars() {
        List<MJExpressionNode> res = new ArrayList<>();
        check(lpar);
    ...
    private MJExpressionNode CondFact() {
        ...
        switch (comp) {
            case ne:
                a = MJBinaryNodeFactory.NotNodeGen.create(a, Expr());
                break;
            case lt:
                a = MJBinaryNodeFactory.LessThanNodeGen.create(a, Expr());
                break;
            case le:
                a = MJBinaryNodeFactory.LessThanEqualNodeGen.create(a, Expr());
                break;
            case eq:
            ...
            case ge:
                ...
                break;
            case gt:
                a = MJBinaryNodeFactory.GreaterNodeGen.create(a, Expr());
                break;
            default:
                break;
        }
    ...
    private MJExpressionNode Expr() {
        ...
        while (sym == plus || sym == minus) {
            if (sym == plus) {
                ...
                Addop();
                expr = MJBinaryNodeFactory.DiffNodeGen.create(expr, Term());
            }
        }
        return expr;
    }
    ...
    private MJExpressionNode Term() {
        ...
        while (sym == times || sym == slash || sym == rem) {
            switch (sym) {
                ...
                case rem:
                    scan();
                    expr = MJBinaryNodeFactory.RemNodeGen.create(expr, Factor());
                    break;
                default:
                    break;
            }
        ...
        switch (sym) {
            ...
            case ident:
                ...
                if (sym == lpar) {
                    ...
                    int index = parameterNames == null ? -1 : parameterNames.indexOf(varname);
                    if (index >= 0) {
            ...
            case number:
                scan();
                int val = t.val;
                expr = MJConstantIntNodeGen.create(val);
                break;
            case charConst:
                scan();
                break;
